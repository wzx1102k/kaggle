...
class SampleClass(object):
    def __init__(self, samples, split_attr=None, split_label={}, calSplitAttrFunc=None, bPackage=False):
        if bPackage == False:
            _real_samples = np.array(samples, dtype='uint8')
            ...
            self.samples = np.vstack((_attr_idx, np.hstack((_sample_idx[1:, ], _real_samples))))
        else:
    ...
    def splitSamples(self):
        ...
        for key in self.split_label:
            if key != -1:
                sub_samples = np.vstack((_samples[0], _valid_samples[attr_values == key, :], _valid_samples[attr_values == -1, :]))
                sub_sample_class[key] = SampleClass(samples=sub_samples, calSplitAttrFunc=self.calSplitAttrFunc, bPackage=True)
            else:
